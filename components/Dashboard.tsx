  ...
  return (
    <Card className="flex-1">
      <div className="flex flex-col gap-2">
        <span className="font-mono text-[10px] md:text-xs tracking-wide-upper text-[var(--foreground)]">
          {label}
        </span>
        <span className="font-mono font-medium text-2xl text-[var(--foreground)]">
          {value}
        </span>
      </div>
    </Card>
  ...
  return (
    <div className="flex gap-2.5 items-start">
      ...
      <div className="w-5 h-5 flex-shrink-0 flex items-center justify-center">
        {isStrain ? (
          <svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="#6E81B7">
      ...
      <p className="font-sans text-xs text-[var(--foreground)] leading-normal flex-1">
        {message}
      </p>
    </div>
  );
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (entry) {
          const parsedInsights = entry.insights ? JSON.parse(entry.insights) : [];
          // Development: Add mock insights if none exist
          const insights = parsedInsights.length > 0 ? parsedInsights : [
            { type: 'strain', message: 'Moderate strain (12.5/21) - solid training session' },
            { type: 'workouts', message: 'Run: 45 min, 165 bpm avg HR, 8.5 km' },
            { type: 'steps', message: 'Excellent step count (15,234)', suggestion: 'Great daily movement baseline' },
            { type: 'readiness', message: 'High readiness - recovered and ready to train', suggestion: 'Can handle high-intensity work' },
          ];
          setData({
            date: entry.date,
            steps: entry.steps,
          ...
          // No entries with data yet - use mock data for development
        ...
        // Set default data on error - use mock data for development
        const today = getTodayDate();
        setData({
          date: today,
          steps: 15234,
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-[#999999] font-mono text-sm">Loading...</div>
      </div>
    );
  }
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex gap-4 items-center">
            ...
            <div className="flex-1 flex flex-col">
              <span className="font-mono font-medium text-[40px] leading-none text-[var(--foreground)]">
                {strainDisplay}
              </span>
              <div className="flex items-center justify-between mt-1">
                <span className="font-mono text-xs tracking-wider-upper text-[var(--foreground)]">
                  of 21 units
                </span>
                {/* Status pill */}
